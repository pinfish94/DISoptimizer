...
df = pd.read_excel('Dataset_all_features_Revisedversion.xlsx', sheet_name='THM')
...
print(f'Number of samples after data cleaning: {len(X)}')
...
print('=' * 60)
print(f"Best 10-fold cross-validation R2: {optimizer.max['target']:.4f}")
...
if seed42_train_pred_df is None or seed42_test_pred_df is None:
    raise ValueError('Failed to save prediction results for seed 42.')
...
print('Included sheets:')
print('- seed42_train_pred: Actual and predicted values for the training set with random seed 42')
print('- seed42_test_pred: Actual and predicted values for the test set with random seed 42')