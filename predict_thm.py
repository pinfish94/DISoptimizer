...
Full pipeline: training, evaluation, model saving, and prediction interface.
...
class CatBoostTHMTrainer:
    ...
    def load_and_preprocess(self) -> None:
        ...
        print(f"Training set: {self.X_train_outer.shape}")
        print(f"Test set: {self.X_test_outer.shape}")
        print("The test set is used only for final evaluation.")
    ...
    def _catboost_cv(
        ...
        if self.X_train_outer is None or self.y_train_outer is None:
            raise ValueError("Call load_and_preprocess() before hyperparameter optimization.")
    ...
    def train_final_model(self, params: Optional[Dict] = None) -> CatBoostRegressor:
        if self.X_train_outer is None or self.y_train_outer is None:
            raise ValueError("Call load_and_preprocess() before training the final model.")
        ...
        print("The final model was trained without early stopping on the test set.")
        return self.model

    def evaluate(self) -> Dict[str, float]:
        if self.model is None:
            raise ValueError("Call train_final_model() before evaluation.")
    ...
    def save_results_excel(self, output_path: str = 'catboost_thm_results.xlsx') -> str:
    ...
    def save_model(self, directory: str = 'saved_models_thm') -> Dict[str, str]:
        if self.model is None:
            raise ValueError("Call train_final_model() before saving the model.")

        os.makedirs(directory, exist_ok=True)

        model_paths = {
            'catboost': os.path.join(directory, 'catboost_thm_model.cbm'),
            'joblib': os.path.join(directory, 'catboost_thm_model.pkl'),
            'metadata': os.path.join(directory, 'thm_model_metadata.json')
        }
    ...
    def full_pipeline(self) -> Dict:
        print("Starting THM training pipeline.")
        print("=" * 60)
        ...
        print("=" * 60)
        print("THM training pipeline completed.")
...
def predict_thm(
    X: pd.DataFrame,
    model_path: str = "saved_models_thm/catboost_thm_model.pkl",
    metadata_path: str = "saved_models_thm/thm_model_metadata.json"
):
    """
    ...
    X : pd.DataFrame
        ...
        ['pH', 'UV254', 'DOC', 'TN', 'Br',
         'Chlorine dose', 'Chlorine consumption', 'EEM_II']
    """
