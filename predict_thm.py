...
class CatBoostTHMTrainer:
    ...
    def load_and_preprocess(self) -> None:
        ...
        if missing_cols:
            raise ValueError(
                f"miss: {missing_cols}\n"
                f"column name: {list(self.df.columns)}"
            )
        ...
        print(f"train: {self.X_train_outer.shape}")
        print(f"test: {self.X_test_outer.shape}")
        print("")
    ...
    def train_final_model(self, params: Optional[Dict] = None) -> CatBoostRegressor:
        ...
        self.model.fit(
            self.X_train_outer,
            self.y_train_outer,
            eval_set=(self.X_test_outer, self.y_test_outer),
            use_best_model=True,
            verbose=False
        )

        print("\nfinal model finished")
        print(f"ℹbest_iteration: {self.model.get_best_iteration()}")
        return self.model
