...
try:
    ...
    print("success in import models")
...
def make_input_for_thm(cl_dose: float, cl_consumed: float, X_wo_dose: dict) -> pd.DataFrame:
    ...
    cols = [
        "pH", "UV254", "DOC", "TN", "Br",
        "EEM_II", "Chlorine dose", "Chlorine consumption"
    ]
    return pd.DataFrame([input_data], columns=cols)
...
def compute_lambda_crit_marginal_area(points, Cmin: float, Csafe: float):
    ...
    if (not points) or (Csafe <= Cmin):
        return 0.0, {"n_used": 0, "A_thm": np.nan, "A_pen": np.nan, "THMmin": np.nan, "THMmax": np.nan}


    band = [p for p in points if (Cmin <= p["free_cl"] <= Csafe) and np.isfinite(p["thm_pred"])]
    if len(band) < 3:
        return 0.0, {"n_used": len(band), "A_thm": np.nan, "A_pen": np.nan, "THMmin": np.nan, "THMmax": np.nan}
...
# ==========================================
def evaluate_dose_with_J(dose: float, X_wo_dose: dict, Csafe: float, a: float, lambda_crit: float, THMmax_for_J: float):
    r = evaluate_dose_raw(dose, X_wo_dose)
    if not r:
        return {"is_feasible": False, "J": 1e9, "cl_dose": float(dose)}
...
def solve_optimization(X_wo_dose: dict, Csafe: float, a: float):
    ...
    lambda_crit, diag = compute_lambda_crit_marginal_area(points, Cmin=HARD_L, Csafe=Csafe)


    THMmax_for_J = diag["THMmax"] if np.isfinite(diag.get("THMmax", np.nan)) else max(p["thm_pred"] for p in points)


    candidates = []
    for p in points:
        r = evaluate_dose_with_J(p["cl_dose"], X_wo_dose, Csafe, a, lambda_crit, THMmax_for_J)
    ...
    def obj(d):
        r = evaluate_dose_with_J(d, X_wo_dose, Csafe, a, lambda_crit, THMmax_for_J)
    ...
    for seed in top:
        ...
        r = evaluate_dose_with_J(res.x, X_wo_dose, Csafe, a, lambda_crit, THMmax_for_J)
        if r and r.get("is_feasible", False):
...
def batch_process(input_file: str, output_file: str, sheet_name: str):
    print(f"starting: {input_file} [{sheet_name}]")
        ...
        if sol:
            ...
            if DIAG_PRINT and (idx < 5):
                print(
                    ...
                    f"A_thm={out['Area_THM_marginal']:.3g}, A_pen={out['Area_penalty']:.3g}, "
                    f"C*={out['Pred_Free_Cl']:.3f}, THM_term={out['THM_term']:.3f}, J={out['Final_J']:.3f}"
                )
        else:
...
if __name__ == "__main__":
    INPUT_EXCEL = "file_name.xlsx"
    SHEET = "sheet_name"

    OUTPUT_EXCEL = f"_C{CSAFE}_a{A_WEIGHT}_.xlsx"
